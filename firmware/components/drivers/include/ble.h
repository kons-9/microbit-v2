...
/**
 * Advertising実装のデバッグ状態。
 *
 * RADIO割り込み・送信完了待ちの状態を、アプリケーションのタスクから
 * 安全に取得するためのスナップショット。
 */
struct AdvertiseDebugStatus {
    uint32_t timer_interrupt_count;
    uint32_t advertise_event_count;
    ...
    uint32_t last_radio_state;
    uint32_t last_radio_events;
    uint32_t last_crcstatus;

    uint32_t radio_power;
    uint32_t radio_frequency;
    uint32_t radio_mode;
    uint32_t radio_pcnf0;
    uint32_t radio_pcnf1;
    uint32_t radio_base0;
    uint32_t radio_prefix0;
    uint32_t radio_crccnf;
    uint32_t radio_crcpoly;
    uint32_t radio_crcinit;
    uint32_t radio_txpower;
    uint32_t radio_tifs;
    uint32_t radio_shorts;
    uint32_t radio_packetptr;
    uint32_t radio_txaddress;
    uint32_t radio_rxaddresses;
    uint32_t radio_datawhiteiv;
    uint32_t radio_config_mismatch;

    /* FICR: 工場書き込み済み・ソフトウェアから設定しない値 */
    uint32_t ficr_part;
    uint32_t ficr_variant;
    uint32_t ficr_package;
    uint32_t ficr_ram;
    uint32_t ficr_flash;
    uint32_t ficr_deviceid0;
