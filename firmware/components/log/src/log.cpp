...
static constexpr const char *const s_level_tags[] = {
    "E", /* ERROR */
    "W", /* WARN  */
    "I", /* INFO  */
    "D", /* DEBUG */
...
void Logger::put_char(char c) {
    if (output_size_ == sizeof(output_buffer_)) {
        flush_output();
    }
    if (output_size_ < sizeof(output_buffer_)) {
        output_buffer_[output_size_++] = static_cast<uint8_t>(c);
    }
}

void Logger::put_str(const char *s) {
    if (s == NULL) {
        put_str("(null)");
        return;
    }
 ...
 */
void Logger::put_uint(uint32_t val, int base, bool upper, int width, bool zero_pad) {
 ...
 * @brief 符号付き整数を10進で出力
 */
void Logger::put_int(int32_t val, int width, bool zero_pad) {
    if (val < 0) {
        put_char('-');
        if (width > 0)
            width--;
...
void Logger::log_vformat(const char *fmt, va_list ap) {
    while (*fmt) {
        if (*fmt != '%') {
            put_char(*fmt++);
            continue;
...
void Logger::init(LogLevel max_level, ::io::Stream &stream) {
    max_level_ = max_level;
    clear_streams();
    add_stream(stream);
}

bool Logger::add_stream(::io::Stream &stream) {
    ...
    if (stream_count_ >= MaxStreams) {
        return false;
    }

    streams_[stream_count_++] = &stream;
    return true;
}
...
void Logger::set_level(LogLevel max_level) {
    max_level_ = max_level;
}

void Logger::output(LogLevel level, const char *tag, const char *fmt, ...) {
    if (level > max_level_) {
        return;
    }
    ...
    /* プレフィックス: [E/TAG] */
    put_char('[');
    put_str(s_level_tags[static_cast<uint8_t>(level)]);
    put_char('/');
    put_str(tag);
    put_char(']');
    put_char(' ');
...
void Logger::hex_dump(LogLevel level, const char *tag, const void *data, size_t len) {
    if (level > max_level_) {
        return;
    }
...
void Logger::flush_output() {
    if (output_size_ == 0) {
        return;
    }

    for (size_t i = 0; i < stream_count_; ++i) {
        streams_[i]->write(output_buffer_, output_size_);
    }
