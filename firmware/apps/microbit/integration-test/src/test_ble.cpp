...
void run_ble_test(Context &context) {
    ...
    drivers::Ble ble_driver;

    auto ble_result = ble_driver.init();
    ...
    auto set_result = ble_driver.gap_advertise_set_data(ad_data, sizeof(ad_data));
    ...
    auto adv_result = ble_driver.gap_advertise_start(ble::AddressType::Public, &adv_params);
    LOG_I("advertise_start: %ld", adv_result);
    context.assert_true(adv_result == 0, "ble::gap_advertise_start");
    context.assert_true(ble_driver.gap_advertise_active() == 1, "ble::gap_advertise_active");

    ble_driver.gap_advertise_stop();
    context.assert_true(ble_driver.gap_advertise_active() == 0, "ble_gap_advertise stopped");
    ...
    auto disc_result
        = ble_driver.gap_discover(ble::AddressType::Public, 2000, &scan_params, ble_scan_callback, nullptr);
